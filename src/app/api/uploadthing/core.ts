...
export const ourFileRouter = {
  ...
  bannerUploader: f({
    ...
    .middleware(async () => {
      ...
      if (!existUser) throw new UploadThingError("Unauthorized");

      if (existUser.bannerKey) {
        const utapi = new UTApi();

        await utapi.deleteFiles(existUser.bannerKey);
        await db
          .update(users)
          .set({ bannerKey: null, bannerUrl: null })
          .where(and(eq(users.id, existUser.id)));
      }
